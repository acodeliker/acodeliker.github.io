...
The `rand` function has some problems: even if not most, there are many programs that require random numbers in different ranges. 
Some applications require random floating point numbers. 
Some programs require non-uniformly distributed random numbers. 
When writing programs to solve these problems, the range, type, or distribution of random numbers generated by rand are often converted, and non-randomness is often introduced.


In C++ 11, the random number library defined in the header file random solves these problems through a set of cooperative classes. Two classes are mainly used:

1) Random-number engines
2) Random-number distribution

Among them, an engine class can generate an unsigned random number sequence, and a distribution uses an engine class to generate a random number of a specified type within a given range and subject to a specified probability distribution.

#### The random number engine 

a class of function objects. They define a call operator that does not accept parameters and returns a random unsigned integer. 
We can generate primitive random numbers by calling a random number engine object.

```cpp
default_random_engine e; // Generate random unsigned numbers, defines a default_random_engine object named e
...
1. Distribution type and engine

In order to get a number within a specified range, we use a distribution type object:

```cpp
/Generate a uniformly distributed random number between 0 and 9 (inclusive)

uniform_int_distribution<unsigned> u(0,9);

default_random_engine e; // Generate unsigned random integer

for (size_t i =0;i<10; i++)
    
// Use u as a random number source
    
// Each call returns a uniformly distributed value within the specified range
...
// The above code input is as follows:
// 0 1 7 4 5 2 0 6 6 9
```

In the above program, we define u as uniform_int_distribution<unsigned>. 
This type generates uniformly distributed unsigned values. 
When we define an object of this type, we can provide the desired minimum and maximum values. 
In the above code, u(0,9) means that we want to get a number between 0 and 9 (inclusive). 
The random number distribution class will use the included range so that we can get every possible value for a given shaping.


Similar to the engine type, the distribution type is also a function object class. 
The distribution type defines a call operator, which accepts a random number engine as a parameter. 
The distribution object uses its engine parameters to generate random numbers and maps them to the specified distribution.


What is passed to the distributed object is the engine object itself, that is, u(e). If we write the call as u(e()), the meaning is to pass the next value generated by e to u, which will cause a compilation 
mistake. 
What we pass is the engine itself, not the next value it generates, because some distributions may require multiple calls to the engine to get a value.

2. Use the engine to generate a sequence of numbers

Random number generators have a feature, that is, even if the generated tree looks random, but for a given generator, it will return the same sequence of values ​​every time the program is run. 
The fact that the sequence does not change is very useful when debugging, but on the other hand, programs that use random number generators must also take this feature into account.


The following introduces an example, which requires a function to generate a vector containing 100 random numbers uniformly distributed between 0 and 9. 
One wrong way is to use the following code:
...
// But every call to this function will return the same vector

vector<unsigned >v1(bad_randVec());

vector<unsigned >v2(bad_randVec());

// will print out equal

cout << ((v1==v2)? "equal": "not equal") << endl;

```

The above code will output equal because vector v1 and v2 have the same value.


The correct definition method is to define the engine and associated distributed objects as static:
...
// Since we want the engine and distributed objects to maintain state, they should be defined as
    
// static, so that each call generates a new number
...
Since both e and u are static, they will remain in state between function calls. 
The first call will use the first 100 random numbers in the sequence generated by u(e), and the second call will get the next 100 random numbers. 
And so on.


Note that a given random number generator is known to generate the same random number sequence. 
If a function defines a local random number generator, it should be defined as static (including engine and distributed objects). 
Otherwise, the same sequence will be generated every time the function is called.


3. Set random number generator seed
The random number generator will generate the same random number sequence. This feature is very useful in debugging. 
However, once our program is debugged, we usually hope that different random results will be generated every time we run the program. This can be achieved by providing a seed. 
The seed is a numerical value, and Yinqin can use it to generate random numbers from a new position in the sequence.

There are two ways to set the seed for the engine:

1) Provide seed when creating engine object

2) Call the seed member of the engine

```cpp
// Almost certainly the wrong way to generate a vector of random integers
    
// Every time this function is called, the same 100 numbers will be generated
...
The most common way to set the seed is to call the system function time. This function is defined in the header file ctime, and it returns how many seconds have passed since a specific moment. 
The function time accepts a single pointer parameter, which points to the data structure used to write the time. 
If this pointer is empty, the function simply returns the time:


default_random_engine e1(time(0)); // a slightly random seed

1

However, since time returns the time in seconds, this method is only suitable for applications where the interval for generating seeds is seconds or longer.

#### Other random number distribution

1. Generate random real numbers

  Programs often require a source of random floating-point numbers. 
In particular, programs often require random numbers between 0 and 1.


  You can use an object of uniform_real_distribution type and let the standard library handle the mapping from random integers to random floating-point numbers. 
As with uniform_int_distribution, when defining the object, we specify the minimum and maximum values.


```cpp    
default_random_engine e; // Generate unsigned random integer
    
// Uniform distribution from 0 to 1 (inclusive)
...
In addition, when we do not specify the default generated type parameter for the distribution function, the program will automatically assign a type. The floating-point value distribution type generates the double type by default, and the integer value distribution type generates the int type by default, as follows:

```cpp
uniform_real_distribution<>u(-1,1); // generate double values ​​by default
```

2. Generate non-uniformly distributed random numbers

In addition to generating the above uniform distribution, C++11 also stipulates that 20 different distribution types can be generated, such as uniform distribution uniform, normal distribution normal, binomial distribution, Poisson distribution poisson, student distribution student, etc. The related functions can be 
